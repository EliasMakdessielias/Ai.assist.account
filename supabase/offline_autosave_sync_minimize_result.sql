-- Etapp 3C-2: DATAMINIMERING av bokslut_sync_comment-resultatet.
-- Tidigare innehöll conflict-svarets serverVersion den faktiska serverkommentaren (comment), som därmed
-- lagrades i bokslut_sync_operations.result_payload i upp till 90 dagar. Detta duplicerade känslig text.
-- Ny modell: serverVersion innehåller ENDAST commentRevision, commentUpdatedAt, commentUpdatedBy,
-- hasServerComment (bool) och serverCommentHash (SHA-256). INGEN kommentartext lagras i operations eller audit.
-- Konflikt-UI hämtar ev. servertext separat via den RLS-skyddade SELECT:en (loadEngagement = medlemskapskontroll),
-- aldrig från det långlivade idempotency-resultatet.
-- (Övrig 3B/3B-1-logik oförändrad – icke-avslöjande not_found/entity_deleted/membership_removed kvarstår.)
...
-- Sanering av befintliga rader: ta bort serverVersion.comment ur lagrade result_payload (bevarar
-- idempotency-resultatet i övrigt). Lägger till hasServerComment + serverCommentHash. No-op om tabellen är tom.
update public.bokslut_sync_operations o
  set result_payload = jsonb_set(
        (o.result_payload #- '{serverVersion,comment}'),
        '{serverVersion}',
        (o.result_payload->'serverVersion') - 'comment'
          || jsonb_build_object(
               'hasServerComment', ((o.result_payload #>> '{serverVersion,comment}') is not null),
               'serverCommentHash', case when (o.result_payload #>> '{serverVersion,comment}') is null then null
                 else encode(extensions.digest(convert_to(normalize(o.result_payload #>> '{serverVersion,comment}', nfc),'UTF8'),'sha256'),'hex') end))
  where o.result_payload ? 'serverVersion'
    and (o.result_payload->'serverVersion') ? 'comment';
