...
export default function AccountingUnderlagPanel({ company, kategori = 'dokument', doc, onSelected, onRemove, dragging = false }) {
  ...
  async function upload(file) {
    ...
    if (!isAllowedFile(file)) { toast.error('Filtypen stöds inte (PDF, JPG, PNG, WEBP)'); return }
    setUploading(true)
    try {
  ...
  return (
    <div className="flex flex-col h-full"
      ...
      <div className="bg-white border-b px-5 h-12 flex items-center gap-3 shrink-0" style={{ borderColor: 'rgba(0,0,0,0.10)' }}>
        ...
        <button className="text-xs text-blue-700 hover:underline flex items-center gap-1" type="button" onClick={() => inputRef.current?.click()} disabled={uploading}>
      ...
      {/* Innehåll: visare när underlag finns, annars lugnt tomt läge */}
      <div className="flex-1 min-h-0 relative">
        {doc ? (
          ...
          <div className="h-full flex flex-col items-center justify-center text-center px-8 bg-white">
            ...
            <div className="text-sm text-gray-500 mb-5">
              Dra och släpp ett underlag här eller klicka på{' '}
              <button type="button" className="text-blue-700 hover:underline" onClick={() => inputRef.current?.click()}>Ladda upp</button>.
            </div>
        ...
        {dragOver && (
          <div className="absolute inset-2 z-10 flex items-center justify-center bg-blue-50/90 border-2 border-dashed border-blue-400 rounded-lg pointer-events-none">
            <div className="text-blue-700 font-medium"><i className="ti ti-upload mr-1" /> Släpp för att ladda upp</div>
          </div>
        )}
      </div>
    </div>
