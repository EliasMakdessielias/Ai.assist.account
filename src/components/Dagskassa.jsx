...
// Kontoplan-koppling (kan göras inställningsbar senare)
const ACC = {
...
export default function Dagskassa() {
  ...
  const [mall, setMall] = useState('exkl') // 'exkl' | 'inkl'
  ...
  const moms = inkl
    ? { 25: s25.moms, 12: s12.moms, 6: s6.moms }
    : { 25: num(f.moms25), 12: num(f.moms12), 6: num(f.moms6) }
  ...
  async function bokfor() {
    ...
    const rows = []
    const credit = (nr, belopp) => { if (belopp > 0.001) rows.push({ nr, debet: 0, kredit: Math.round(belopp * 100) / 100 }) }
    const debit = (nr, belopp) => { if (belopp > 0.001) rows.push({ nr, debet: Math.round(belopp * 100) / 100, kredit: 0 }) }
    ...
    try {
      ...
      if (e2) throw e2
      // Aktivera ev. inaktiva konton som användes.
  ...
  const Row = ({ label, k, locked, value }) => (
    <div className="grid grid-cols-[200px_220px] items-center gap-3 mb-2">
      <label className="text-sm text-gray-600">{label}</label>
      <input className="input text-right" inputMode="decimal" disabled={locked}
        style={{ opacity: locked ? 0.6 : 1 }}
        value={locked ? (value ? fmt(value) : '') : (f[k] ?? '')}
  ...
  return (
    <div className="max-w-3xl">
      <div className="flex items-center gap-4 mb-5">
        <span className="text-[15px] font-bold tracking-tight">NY FÖRSÄLJNING {inkl ? 'INKL.' : 'EXKL.'} MOMS</span>
      </div>
      ...
      <div className="grid grid-cols-[1fr_1fr] gap-4 mb-5 max-w-xl">
        <div>
          ...
          <select className="input" value={mall} onChange={e => setMall(e.target.value)}>
        ...
        <div>
          ...
          <input className="input" type="date" value={datum} max={today} onChange={e => setDatum(e.target.value)} />
        </div>
        <div className="col-span-2">
          <label className="block text-xs font-medium text-gray-500 mb-1">Verifikationsbeskrivning</label>
          <input className="input" value={beskrivning} onChange={e => setBeskrivning(e.target.value)} />
        </div>
      </div>

      <div className="mb-5">
        <div className="text-sm font-semibold mb-2">Försäljning {inkl ? 'inkl.' : 'exkl.'} moms</div>
        <Row label="Varugrupp 25% moms" k="vg25" />
        <Row label="Varugrupp 12% moms" k="vg12" />
        <Row label="Varugrupp 6% moms" k="vg6" />
        <Row label="Varugrupp momsfri" k="vg0" />
        <div className="grid grid-cols-[200px_220px] gap-3"><span /><div className="text-right text-sm text-gray-500 pr-1">Netto: <b className="text-gray-800 tabular-nums">{fmt(salesTotal)}</b></div></div>
      </div>
      ...
      <div className="mb-5">
        ...
        <Row label="Moms 25%" k="moms25" locked={inkl} value={moms[25]} />
        <Row label="Moms 12%" k="moms12" locked={inkl} value={moms[12]} />
        <Row label="Moms 6%" k="moms6" locked={inkl} value={moms[6]} />
        <div className="grid grid-cols-[200px_220px] gap-3"><span /><div className="text-right text-sm text-gray-500 pr-1">Moms: <b className="text-gray-800 tabular-nums">{fmt(momsTotal)}</b></div></div>
      </div>

      <div className="mb-5">
        <div className="text-sm font-semibold mb-2">Betalsätt</div>
        <Row label="Kontant" k="kontant" />
        <Row label="Kort" k="kort" />
        <div className="grid grid-cols-[200px_220px] gap-3"><span /><div className="text-right text-sm text-gray-500 pr-1">Inbetalt: <b className="text-gray-800 tabular-nums">{fmt(payments)}</b></div></div>
      </div>
      ...
      <div className="flex gap-3">
        ...
        <button className="btn btn-green px-6" onClick={bokfor} disabled={saving}>{saving ? 'Bokför…' : 'Bokför'}</button>
      </div>
    </div>
  )
}
