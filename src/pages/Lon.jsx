    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-3">
          <label className="text-sm text-gray-500 flex items-center gap-1.5 cursor-pointer">
...
// Lönekörningar byggs i nästa steg (kräver Anställda-registret som nu finns).
function Lonekorningar() {
  return (
    <div className="bg-white rounded-xl p-12 text-center text-gray-400" style={{ border: '0.5px solid rgba(0,0,0,0.10)' }}>
      ...
      <div className="text-sm">Lägg till dina anställda under fliken <b>Anställda</b> först – sedan kan vi köra lön (period → anställda → granska &amp; bokför).</div>
    </div>
  )
}

export default function Lon() {
  const [tab, setTab] = useState('anstallda')
  const tabs = [['anstallda', 'Anställda'], ['lonekorningar', 'Lönekörningar']]
  return (
    <div>
      <div className="bg-white border-b sticky top-0 z-10 px-7 h-14 flex items-center justify-between" style={{ borderColor: 'rgba(0,0,0,0.10)' }}>
        <span className="text-base font-medium">Lön</span>
      </div>
      <div className="px-7 pt-4">
        <div className="flex gap-1 border-b" style={{ borderColor: 'rgba(0,0,0,0.10)' }}>
          {tabs.map(([key, label]) => (
            <button key={key} onClick={() => setTab(key)}
              className={`px-4 py-2 text-sm font-medium border-b-2 -mb-px ${tab === key ? 'text-gray-900 border-blue-700' : 'text-gray-500 border-transparent hover:text-gray-700'}`}>
              {label}
            </button>
          ))}
        </div>
      ...
      <div className="p-7">
        {tab === 'anstallda' ? <Anstallda /> : <Lonekorningar />}
      </div>
    </div>
  )
}
